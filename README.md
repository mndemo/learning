...
<th:block th:fragment="yesNoRadioWithHouseholdFollowup (input, data)" 
	...
	<div class="form-group" th:classappend="${hasError } ? 'form-group--error' : ''">
		...
		<div class="question-with-follow-up">
			...
			<!-- When Yes is selected but there are NO household members: submit "applicant" so SELECT_AT_LEAST_ONE_IF_YES_SELECTED passes (size > 1). -->
			<th:block th:if="${isYesSelected and !hasHouseholdMembers}" th:with="applicantFullName=${input.options.datasources.get('personalInfo').get('firstName').value[0] + ' ' + input.options.datasources.get('personalInfo').get('lastName').value[0]}">
