...
public class MnitDocumentConsumer {
  ...
  private final String uploadedDocumentsDeliveryFailureAlertRecipient;
  ...
  public MnitDocumentConsumer(XmlGenerator xmlGenerator,
      ...
      @Value("${uploaded-documents-delivery-failure-alert.recipient-email:}") String uploadedDocumentsDeliveryFailureAlertRecipient) {
    ...
    this.uploadedDocumentsDeliveryFailureAlertRecipient = uploadedDocumentsDeliveryFailureAlertRecipient;
  }
  ...
  public void processUploadedDocuments(Application application) {
    ...
    if (combinedUploadedFiles.isEmpty()
        || (combinedUploadedFiles.stream().allMatch(uDoc -> uDoc.getFileBytes() == null))) {
	      log.error(
	          "There was an issue processing and delivering uploaded documents. Reach out to client to upload again. applicationId={}",
	          application.getId());
	      applicationStatusRepository.createOrUpdateAllForDocumentType(application,
	          UNDELIVERABLE, UPLOADED_DOC);
	      sendUploadedDocumentsDeliveryFailureAlert(application);
	      return;
	    }
  ...
  /**
   * Sends an ops email when uploaded documents cannot be prepared for delivery (empty or unreadable
   * payload). Set {@code uploaded-documents-delivery-failure-alert.recipient-email} (or env
   * {@code UPLOADED_DOCUMENTS_DELIVERY_FAILURE_ALERT_EMAIL}) to enable; leave blank to skip.
   */
  private void sendUploadedDocumentsDeliveryFailureAlert(Application application) {
    if (!StringUtils.hasText(uploadedDocumentsDeliveryFailureAlertRecipient)) {
      return;
    }
    String applicationId = application.getId();
    try {
      String subject = "[MNbenefits] Uploaded documents could not be delivered — application %s"
          .formatted(applicationId);
      String body =
          "<p>There was an issue processing and delivering uploaded documents. Please reach out to the client so they can upload again.</p>"
              + "<p><strong>Application ID:</strong> %s</p><p><strong>Flow:</strong> %s</p>"
              .formatted(applicationId, application.getFlow());
      emailClient.sendEmail(
          subject,
          senderEmail,
          uploadedDocumentsDeliveryFailureAlertRecipient.trim(),
          body,
          applicationId);
    } catch (Exception e) {
      log.error(
          "Failed to send uploaded-documents delivery failure alert email for application {}",
          applicationId,
          e);
    }
  }
